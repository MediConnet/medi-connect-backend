...
export async function getAllDoctors(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResult> {
  ...
  try {
    ...
    const formattedDoctors = doctors.map(mapDoctorData);
    
    console.log(`✅ [PUBLIC DOCTORS] Retornando ${formattedDoctors.length} médicos.`);
 ...
 * Centraliza la lógica para que getAll, search y getById devuelvan lo mismo.
 */
function mapDoctorData(doctor: any) {
    ...
    return {
      ...
      calificacion: mainBranch?.rating_cache ? Number(mainBranch.rating_cache) : 0,
